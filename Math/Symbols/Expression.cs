...
using LSharp.Math.Comparison.EqualityComparison;
using LSharp.Math.Evaluation;
using LSharp.Math.Evaluation.ConstantEvaluation;
using LSharp.Math.Manipulation;
using LSharp.Math.Manipulation.SummationManipulation;
using LSharp.Math.Manipulation.MultiplicationManipulation;
using LSharp.Math.Manipulation.DivisionManipulation;
...
namespace LSharp.Math.Symbols
{
    public class Expression
    {
        ...
        */
        ...
        {
            Expression copy = CopyTree();

            if (instruction == ComparisonInstruction.IsEqual)
            {
                comparisonStrategy = new IsEqualStrategy(copy, other, first, second);
            }
            else if (instruction == ComparisonInstruction.IsEqualByBase)
            {
                comparisonStrategy = new IsEqualByBaseStrategy(copy, other, first, second);
            }
            else
            {
        ...
        public Expression Evaluate(EvaluationInstruction instruction)
        {
            ...
            {
                evaluationStrategy = new ConstantEvaluationStrategy(copy);
            }
            else
            {
                throw new Exception("Instruction not recognised");
        ...
        public Expression Manipulate(ManipulationInstruction instruction)
        {
            ...
            {
                manipulationStrategy = new LikeTermStrategy(copy);
            }
            else if (instruction == ManipulationInstruction.Distribute)
            {
                manipulationStrategy = new DistributionStrategy(copy);
            }
            else if (instruction == ManipulationInstruction.Cancel)
            {
                manipulationStrategy = new CancellationStrategy(copy);
            }
            else
            {
                throw new Exception("Instruction not recognised");
        ...
        public Expression ApplyRule(RuleApplicationInstruction instruction)
        {
            Expression copy = CopyTree();

            if (instruction == RuleApplicationInstruction.ExponentRuleOne)
            {
                rule = new ExponentRuleOne(copy);    
            }
            else if (instruction == RuleApplicationInstruction.ExponentRuleTwo)
            {
                rule = new ExponentRuleTwo(copy);  
            }
            else if (instruction == RuleApplicationInstruction.ExponentRuleThree)
            {
                rule = new ExponentRuleThree(copy);  
            }
            else if (instruction == RuleApplicationInstruction.ExponentRuleFour)
            {
                rule = new ExponentRuleFour(copy);  
            }
            else if (instruction == RuleApplicationInstruction.ExponentRuleFive)
            {
                rule = new ExponentRuleFive(copy);  
            }
            else if (instruction == RuleApplicationInstruction.ExponentRuleSix)
            {
                rule = new ExponentRuleSix(copy);  
            }
            else
            {
                throw new Exception("Instruction not recognised");
            }
            Expression result = rule.Apply();
